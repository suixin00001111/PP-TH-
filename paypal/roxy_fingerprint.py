...
def _normalize_roxy_language(value: object, *, default: str = "en-US") -> str:
    """Map protocol locales to values accepted by Roxy Local API.

    Probe (Roxy Local API): th-TH fails, th works; ja-JP works, ja fails;
    ko-KR fails. Prefer explicit aliases, then original, then primary tag, then en-US.
    """
    ...
    aliases = {
        ...
        "ja": "ja-JP",
        "ja-jp": "ja-JP",
        "ko": "en-US",  # Roxy build rejected ko-KR in probe; safe fallback
        "ko-kr": "en-US",
        "pt": "pt-BR",
        "pt-br": "pt-BR",
        "zh": "zh-CN",
        "zh-cn": "zh-CN",
        "zh-hans": "zh-CN",
        "zh-tw": "zh-TW" if False else "zh-CN",
        "en": "en-US",
        "en-us": "en-US",
        "en-gb": "en-GB",
        "vi": "vi-VN",
    ...
    if low in aliases:
        return aliases[low]
    # keep well-formed xx-YY as-is (ja-JP, en-US, pt-BR, ...)
    parts = raw.split("-")
...
def _roxy_language_candidates(value: object) -> list[str]:
    primary = _normalize_roxy_language(value)
    raw = str(value or "").strip().replace("_", "-")
    out: list[str] = []
    for item in (primary, raw, raw.split("-")[0].lower() if raw else "", "en-US", "en"):
...
def _roxy_proxy_info(proxy_url: str, *, allow_noproxy: bool = True) -> dict[str, Any]:
    ...
    # Prefer project ProxyEntry parser (host:port:user:pass, user:pass@host:port, URLs…)
    try:
        from paypal.proxy import ProxyEntry

        entry = ProxyEntry.parse(value)
        host = entry.host
        port = int(entry.port)
        username = entry.username or ""
        password = entry.password or ""
        scheme = (entry.scheme or "http").lower()
    except Exception:
        try:
    ...
    info = {
        ...
        # Some Roxy builds also read these aliases
        "proxyHost": host,
        "proxyPort": str(port),
...
def load_roxy_capture_config(proxy_url: str | None = None) -> RoxyCaptureConfig:
    ...
    return RoxyCaptureConfig(
        ...
        language=_normalize_roxy_language(_env_str("PAYPAL_ROXY_LANGUAGE", language) or language),
        display_language=_normalize_roxy_language(
            _env_str("PAYPAL_ROXY_DISPLAY_LANGUAGE", language) or language
        ),
    ...
    def create_profile(self, workspace_id: int, project_id: int | None) -> str:
        ...
        if str(proxy_info.get("proxyCategory") or "").lower() == "noproxy":
            logger.warning(
                ...
                "If you enabled proxy in Web UI, the URL did not reach Roxy."
            )
        else:
            logger.info(
                "Roxy profile proxy {}:{} category={}",
                proxy_info.get("host"),
                proxy_info.get("port"),
        ...
        payload: dict[str, Any] = {
            ...
            "fingerInfo": {
                ...
                "language": _normalize_roxy_language(self.config.language),
                "isDisplayLanguageBaseIp": self.config.follow_ip,
                "displayLanguage": _normalize_roxy_language(self.config.display_language),
