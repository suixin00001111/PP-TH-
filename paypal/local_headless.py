...
def _playwright_proxy_config(proxy_url: str) -> JsonObject:
    """Build Playwright proxy options.

    Chromium cannot do SOCKS5 *with* username/password. When the resolved outbound
    proxy is socks5(+auth), open a local HTTP CONNECT bridge that forwards through
    the same filled host/user/pass, and point Playwright at 127.0.0.1.
    """
    try:
    ...
    if scheme in {"socks5", "socks5h"} and (username or password):
        try:
            from paypal.proxy_bridge import bridge_from_proxy_url

            bridge = bridge_from_proxy_url(proxy_url)
            # Keep bridge alive for the process lifetime of this browser session.
            bridges = globals().setdefault("_PLAYWRIGHT_SOCKS_BRIDGES", [])
            bridges.append(bridge)
            local = urlsplit(bridge.http_proxy_url)
            logger.info(
                "Local headless: SOCKS5 auth -> local HTTP bridge {} (upstream {}://{}:***@{}:{})",
            ...
            return {"server": f"http://{local.hostname}:{local.port}"}
        except Exception as exc:
            logger.warning(
                "Local headless: failed to start SOCKS5 bridge ({}); "
                "falling back to HTTP scheme on same host (may fail if provider blocks CONNECT): {}",
                exc,
                proxy_url.split("@")[-1] if "@" in proxy_url else proxy_url,
            )
            scheme = "http"

    server = urlunsplit((scheme, host, "", "", ""))
    proxy: JsonObject = {"server": server}
    if username and scheme in {"http", "https"}:
        proxy["username"] = username
    if password and scheme in {"http", "https"}:
