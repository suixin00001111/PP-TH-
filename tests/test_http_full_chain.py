...
class HttpFullChainTests(unittest.TestCase):
    def test_region_headers_are_thai(self):
        headers = build_common_headers("TH", "th-TH")
        self.assertIn("th-TH", headers["Accept-Language"])
        session = PayPalSession(SessionState(region="TH"))
        try:
            self.assertEqual(session.country, "TH")
            self.assertEqual(session.locale_tag, "th_TH")
            self.assertEqual(session.client.headers["Accept-Language"].split(",", 1)[0], "th-TH")
        finally:
            session.close()
